...
import { Heart, Download, Clock, CheckCircle, XCircle, Users } from 'lucide-react';
import AdminHeader from '../../components/AdminHeader';
import AdminVolunteerStats from '../../components/AdminVolunteerStats';
import AdminVolunteerList from '../../components/AdminVolunteerList';
import { 
  getVolunteerApplicationsWithFilters, 
  updateVolunteerApplicationStatus, 
  exportVolunteerApplications 
} from '../../../volunteer-application/actions';
...
export default function AdminVolunteerApplicationsClient() {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        const result = await getVolunteerApplicationsWithFilters('all', 'all', 1, 1000);
        if (result.success) {
  ...
  const handleStatusUpdate = async (applicationId: string, newStatus: VolunteerApplicationStatus) => {
    try {
      const result = await updateVolunteerApplicationStatus(applicationId, newStatus);
      if (result.success) {
        ...
        // Refresh stats
        const updatedResult = await getVolunteerApplicationsWithFilters('all', 'all', 1, 1000);
  ...
  const handleExport = async () => {
    ...
    try {
      const result = await exportVolunteerApplications(currentFilter, 'all');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        <AdminVolunteerStats stats={stats} isLoading={isLoading} />
